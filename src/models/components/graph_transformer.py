...
class GraphTransformer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.layers = List([])
        edge_dim = default(edge_dim, latent_dim)
        self.norm_edges = nn.LayerNorm(edge_dim) if norm_edges else nn.Identity()

        self.adj_emb = nn.Embedding(num_edge_types, edge_dim) if accept_adjacency_matrix else None
        ...
        for _ in range(num_layers):
            self.layers.append(List([
                List([
                    PreNorm(latent_dim, Attention(latent_dim, pos_emb = pos_emb, edge_dim = edge_dim, dim_head = dim_head, heads = heads)),
                    GatedResidual(latent_dim)
                ]),
                List([
